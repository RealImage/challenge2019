...
func run(partners, input, output, capacities string) error {
	...
	if capacities != "" {
		...
		if err != nil {
			return fmt.Errorf("unable to read partner capacities from capacity file %q, err: %w", capacities, err)
		}
	}

	res := ProcessData(rates, req, cap)

	if err := WriteData(res, output); err != nil {
...
func ProcessData(rates []*Rate, requests []*DeliveryRequest, capacities map[string]int) []*DeliveryResponse {
...
func updateData(rate *Rate, req *DeliveryRequest, r *DeliveryResponse) {
	if rate.Theatre == req.Theatre && req.Size >= rate.Lower && req.Size <= rate.Upper {
...
func WriteData(rr []*DeliveryResponse, file string) error {
