...
import(
	...
	"strconv"
 	"io"
 	"log"
	"github.com/funcoding/qube-challenge-2019/helpers"
...
func createPartnerHashTable() Partner {
	csvPath, csvPathErr := filepath.Abs("static/partners.csv")
	if csvPathErr != nil{
...
func createCapacityHashTable() Capacity{
	csvPath, csvPathErr := filepath.Abs("static/capacities.csv")
	if csvPathErr != nil{
		log.Fatal(csvPathErr)
	}
	r := helpers.ParseContentsFromCsv(csvPath)
