...
def check_capacity(input,ans)
  limit=limits.select{|limit| limit[0].include(ans)}
  return limit[0][1].to_i>input[1]
end
result = []
...
input_data.map do |input|
	...
	capacity=ans.empty? ? check_capacity(input,ans[0][:partner_id]) : false
    unless ans.empty? && capacity
...
CSV.open("/home/development/Challenges/output2.csv", "w") do |csv|
